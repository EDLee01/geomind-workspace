...
├── MCP Layer: filesystem / paper-search / Materials Project / FRED / Zotero / GitHub / custom
...
First batch: `filesystem` (project files), `paper-search-mcp` (literature),
`Materials Project` (material properties/structures), `FRED` (economic time series),
`Zotero MCP` (library), `GitHub MCP` (repos/issues/releases), `local runtime MCP`
(execution status). v1 ships filesystem + paper search first; the rest follow.
...
9. Build static artifacts for the climate demo.
